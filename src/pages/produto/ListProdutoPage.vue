<template>
  <div class="q-pa-md">
    <q-table
      title="Produtos"
      :rows="produtos"
      :columns="columns"
      color="primary"
      row-key="index"
      class="q-ma-sm"
      label="Show Loading"
    >

      <template v-slot:body-cell-img_url="props">
          <q-td :props="props">
            <q-avatar>
              <img :src="props.row.img_url" />
            </q-avatar>
          </q-td>
      </template>

      <template v-slot:top-right>
        <div class="q-gutter-x-sm">
          <q-btn
            v-if="$q.platform.is.desktop"
            outline
            icon="mdi-plus"
            label="Nova"
            color="primary"
            dense
            no-caps
            to="/register-produto"
          />
          <q-btn
            outline
            dense
            color="primary"
            icon-right="archive"
            no-caps
            @click="exportTable"
          />
        </div>
      </template>

      <template v-slot:body-cell-actions="props">
        <q-td :props="props" class="q-gutter-x-sm">
          <q-btn color="info" icon="mdi-pencil-outline" :to="{name:'register-produto',params:{id:props.row.id}}" dense>
            <q-tooltip
              transition-show="rotate"
              transition-hide="rotate"
            >
              Edit
            </q-tooltip>
          </q-btn>
          <q-btn color="negative" icon="mdi-delete-outline" @Click="handleDeletarProduto(props.row)" dense>
            <q-tooltip
              transition-show="rotate"
              transition-hide="rotate"
              dense
            >
              Remove
            </q-tooltip>
          </q-btn>
        </q-td>
      </template>
    </q-table>
  </div>
  <q-page-sticky to="/register-produto"
  position="bottom-right"
  :offset="[18, 18]">
    <q-btn
      v-if="$q.platform.is.mobile"
      :to="{name:'register-produto'}"
      fab
      icon="add"
      color="primary"
    />
  </q-page-sticky>
</template>

<script>
import { exportFile, useQuasar } from 'quasar'
import { defineComponent, ref, onMounted} from 'vue'
import useApi from 'src/composables/useApi'
import useNotify from 'src/composables/useNotify'
import { useRouter } from 'vue-router'

const columns = [
  {
    name: 'index',
    required: true,
    label: '#',
    field: 'index',
    align: 'left',
  },
   {
    name: 'img_url',
    required: true,
    label: 'Foto',
    align: 'left',
     field: row => row.img_url,
    // format: val => `${val}`,
  },
  {
    name: 'name',
    required: true,
    label: 'Name',
    align: 'left',
     field: row => row.name,
    // format: val => `${val}`,
  },
   {
    name: 'quantidade',
    required: true,
    label: 'Quantidade',
    align: 'left',
     field: row => row.quantidade,
    // format: val => `${val}`,
  },
   {
    name: 'preco',
    required: true,
    label: 'Preço',
    align: 'left',
     field: row => row.preco,
    // f,
    ormat: val => `${val}`,
  },
   {
    name: 'actions',
    required: true,
    label: 'Actions',
    align: 'right',
     field: row => row.actions,
    // format: val => `${val}`,
  }
 ]

function wrapCsvValue (val, formatFn, row) {
  let formatted = formatFn !== void 0
    ? formatFn(val, row)
    : val

  formatted = formatted === void 0 || formatted === null
    ? ''
    : String(formatted)

  formatted = formatted.split('"').join('""')
  /**
   * Excel accepts \n and \r in strings, but some other CSV parsers do not
   * Uncomment the next two lines to escape new lines
   */
  // .split('\n').join('\\n')
  // .split('\r').join('\\r')

  return `"${formatted}"`
}

export default defineComponent ({
  name:'ListProdutoPage',
  setup () {
    const $q = useQuasar()
    const {list,remove} = useApi()
    const {semSucesso,sucesso} = useNotify()
    const produtos = ref([])
    const router = useRouter()
    const table = 'produto'

    const handleListProdutos = async()=>{
      try{
        produtos.value = await list(table)
        produtos.value.forEach((row, index) => {
          row.index = index
        })
        $q.loading.hide()
      }catch(error){
          $q.loading.hide()
          semSucesso(error.message)
      }
    }

    onMounted(()=>{
      $q.loading.show()
      handleListProdutos()
    })

    const handleDeletarProduto = (produto) => {
       $q.dialog({
            title:"Atecao",
            message:`Desejas deletar a produto ${produto.name}?`,
            cancel:true,
            persistent: true
          }).onOk(async()=>{
              try{
                  await remove(table,produto.id)
                  produtos.value.splice(produtos.value.indexOf(produto),1)
                  sucesso(`Produto ${produto.name} deletada`)
              }catch(error){
                semSucesso('Falha na remocao da produto '+error.message)
              }
          })
    }

    return {
      columns,
      handleDeletarProduto,
      produtos,
      router,
      exportTable () {
        // naive encoding to csv format
        const content = [columns.map(col => wrapCsvValue(col.label))].concat(
          rows.map(row => columns.map(col => wrapCsvValue(
            typeof col.field === 'function'
              ? col.field(row)
              : row[ col.field === void 0 ? col.name : col.field ],
            col.format,
            row
          )).join(','))
        ).join('\r\n')

        const status = exportFile(
          'table-export.csv',
          content,
          'text/csv'
        )

        if (status !== true) {
          $q.notify({
            message: 'Browser denied file download...',
            color: 'negative',
            icon: 'warning'
          })
        }
      }
    }
  }
})
</script>
